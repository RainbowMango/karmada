...
type CustomizationRules struct {
	...
	// ComponentReplicaResource describes the rules for Karmada to discover the resource requirements
...
// ComponentReplicaResourceRequirement holds the Lua script for extracting the desired replica count
...
type ComponentReplicaResourceRequirement struct {
	...
	//       to the member cluster.
	//
	// The function expects one return values:
	//   - components: resource requirements for each component.
	// The returned values will be set into a ResourceBinding or ClusterResourceBinding.
	// +required
	LuaScript string `json:"luaScript"`
}
